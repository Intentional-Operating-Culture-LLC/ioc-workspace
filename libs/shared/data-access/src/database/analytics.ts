...
import { DatabaseAnalyticsEvent as AnalyticsEvent, GetAnalyticsOptions as AnalyticsOptions } from "@ioc/shared/types";
 ...
 */
export async function getAnalyticsSummary(organizationId: string, dateRange: number = 30): Promise<AnalyticsSummary> {
    ...
    const { data: uniqueUsersData } = await supabase
        ...
        .gte('created_at', startDate.toISOString())
        .lte('created_at', endDate.toISOString());
    const uniqueUsers = new Set(uniqueUsersData?.map((e: any) => e.user_id).filter(Boolean)).size;
 ...
 * Get events by type
 */
export async function getEventsByType(organizationId: string, eventType: string, options: AnalyticsOptions = {}) {
 ...
 */
export async function getTimeSeriesData(organizationId: string, options: AnalyticsOptions = {}) {
    const endDate = options.endDate ? new Date(options.endDate) : endOfDay(new Date());
    const startDate = options.startDate
        ? new Date(options.startDate)
        : startOfDay(subDays(endDate, options.days || 30));
    ...
    const eventTypeData: Record<string, Record<string, number>> = {};
 ...
 */
export async function getUserEngagementMetrics(organizationId: string, options: AnalyticsOptions = {}) {
    const endDate = options.endDate ? new Date(options.endDate) : endOfDay(new Date());
    const startDate = options.startDate
        ? new Date(options.startDate)
        : startOfDay(subDays(endDate, options.days || 30));
    ...
    const engagementStats = {
        ...
        avg_days_active: 0,
        daily_active_users: {} as Record<string, number>,
        user_retention: {},
    };
    // Average events per user
    const totalEvents = Object.values(userMetrics).reduce((sum: number, user: any) => sum + user.event_count, 0);
    engagementStats.avg_events_per_user = engagementStats.total_users > 0
        ? totalEvents / engagementStats.total_users
        : 0;
    // Average days active
    const totalDaysActive = Object.values(userMetrics).reduce((sum: number, user: any) => sum + user.days_active.size, 0);
 ...
 */
export async function getFunnelAnalytics(organizationId: string, funnelSteps: string[], options: AnalyticsOptions = {}) {
    const endDate = options.endDate ? new Date(options.endDate) : endOfDay(new Date());
    const startDate = options.startDate
        ? new Date(options.startDate)
        : startOfDay(subDays(endDate, options.days || 30));
    ...
    const userProgression: Record<string, any> = {};
    events?.forEach((event: any) => {
        const userId = event.user_id;
    ...
    const funnelMetrics = funnelSteps.map((step, index) => {
        const usersAtStep = Object.values(userProgression).filter((user: any) => user.steps_completed.has(index)).length;
        const usersCompletedStep = Object.values(userProgression).filter((user: any) => user.max_step >= index).length;
        return {
            step,
            step_index: index,
    ...
    return {
        ...
        completed_funnel: Object.values(userProgression).filter((user: any) => user.max_step === funnelSteps.length - 1).length,
    };
}
/**
 * Get assessment analytics
 */
export async function getAssessmentAnalytics(organizationId: string, assessmentId?: string) {
    let query = supabase
    ...
    const analytics = {
        ...
        completed: responses?.filter((r: any) => r.status === 'submitted').length || 0,
        in_progress: responses?.filter((r: any) => r.status === 'in_progress').length || 0,
        completion_rate: 0,
        avg_time_to_complete: 0,
        by_assessment: {} as Record<string, any>,
    };
