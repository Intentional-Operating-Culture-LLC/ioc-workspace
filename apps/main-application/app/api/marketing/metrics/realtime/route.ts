...
export async function GET(request: NextRequest) {
    try {
        ...
        for (const metricType of metricTypes) {
            const typeMetrics = metricsData.filter(m => m.metric_type === metricType);
            // Calculate current (last 30 min) and previous (30-60 min ago) values
            const currentMetrics = typeMetrics.filter(m => new Date(m.created_at) >= thirtyMinutesAgo);
            const previousMetrics = typeMetrics.filter(m => new Date(m.created_at) < thirtyMinutesAgo && new Date(m.created_at) >= oneHourAgo);
            ...
            for (let i = 0; i < 6; i++) {
                ...
                const intervalMetrics = currentMetrics.filter(m => {
                    const metricTime = new Date(m.created_at);
                    return metricTime >= intervalStart && metricTime < intervalEnd;
                });
                const intervalValue = intervalMetrics.reduce((sum, m) => sum + m.value, 0);
                trend.unshift({
